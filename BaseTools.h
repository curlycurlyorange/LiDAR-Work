...
namespace LiDARBaseTools{
	...
	/*
	todo:      read txt point data to a LasPoint vector（inputCloud）
	date:      2015.3.8
	...
	*/	
	...
	/*
	//一个容器里面有许多整型容器std::vector<std::vector<int>> &cluster_result
	//cluster_result是一组整型容器
	...
	*/
	...
	bool Ruster(std::vector<std::vector<int>> &cluster_raster,//每一个空值聚类结果
		std::vector<std::vector<int>> &rust_raster,//每一个腐蚀区
		std::vector<std::vector<int>> &boundary_raster,//每一个腐蚀边界
		const int nrows,const int ncols)
	{
